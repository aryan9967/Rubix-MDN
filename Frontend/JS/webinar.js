...
//#2
let config = {
    appid:"8036bbbc530448deb8649924d0ce1dbe",
    token:"007eJxTYOCb0sXz++BKyU73GxmPfs1qkJniVmL52DX+hqHKV6vYuo0KDBYGxmZJSUnJpsYGJiYWKalJFmYmlpZGJikGyamGKUmpIg9XpDYEMjI88r/IwAiFID4rQ0ZqTk4+AwMAjukgww==",
    uid:null,
    channel:"hello",
}
...
function updateGridTemplate() {
    const userStreamsContainer = document.getElementById('user-streams');
    const numberOfUsers = Object.keys(remoteTracks).length + (localTracks.videoTrack ? 1 : 0);

    // Toggle the 'one-user' class based on the number of users
    userStreamsContainer.classList.toggle('one-user', numberOfUsers === 1);
}
...
let joinStreams = async () => {
    ...
    client.on("volume-indicator", function(evt){
        for (let i = 0; evt.length > i; i++){
            ...
            if(volume > 0){
                document.getElementById(`volume-${speaker}`).src = './images/volume-on.svg'
            }else{
                document.getElementById(`volume-${speaker}`).src = './images/volume-off.svg'
            }
